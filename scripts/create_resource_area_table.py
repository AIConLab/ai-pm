...
#!/usr/bin/env python3
# file: create_resource_area_table.py
# desc: Inits datatable for resource area items - VERIFICATION MODE
...
def analyze_resource_area(schematic_file):
    """Analyze resource area schematic and print results for verification"""
    print(f"\n=== ANALYZING resource AREA ===")
    
    # Blocks to exclude from the resource area inventory
    ...
    try:
        ...
        print(f"\n=== FINAL resource AREA ITEMS ===")
        print(f"Remaining unique items (blocks + chest contents): {len(filtered_blocks)}")
        
        if filtered_blocks:
            print("\nFinal resource area inventory (unique items only):")
            ...
            print("Items that would be stored in resourceArea table:")
            for item in sorted(filtered_blocks):
                item_type, attributes = parse_block_state(item)
                source = []
                if item in unique_blocks:
            ...
            print("⚠️  No items found for resource area after filtering")
        ...
        print(f"❌ Error analyzing resource area: {e}")
        import traceback
        traceback.print_exc()
        return set(), set(), set()  # Return empty sets on error


def insert_resource_area_items(db, filtered_blocks, unique_blocks, chest_items):
    """Insert resource area items into the resourceArea table"""
    with db.get_connection() as conn:
        ...
        for item in filtered_blocks:
            ...
            cursor.execute("""
                INSERT OR REPLACE INTO resourceArea 
...
if __name__ == "__main__":
    # Process the resource area schematic
    schem_path = "/mc-data/plugins/WorldEdit/schematics/resource_area.schem"
    filtered_blocks, unique_blocks, chest_items = analyze_resource_area(schem_path)
    ...
    # Clear existing resource area data
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM ResourceArea")
        print("🧹 Cleared existing resource area data")
    
    insert_resource_area_items(db,
